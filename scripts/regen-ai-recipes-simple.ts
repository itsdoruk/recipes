...
async function generateAIRecipeFromTheMealDB() {
  try {
    ...
    const newRecipe = {
      ...
      description: meal.strInstructions ? meal.strInstructions.substring(0, 200) + '...' : 'A delicious recipe to try!',
      ingredients,
      instructions,
...
async function main() {
  ...
  // 2. Generate 120 new AI recipes from TheMealDB
  console.log('Generating 120 AI recipes from TheMealDB...');
  for (let i = 0; i < 120; i++) {
    try {
      ...
      console.log(`Inserted AI recipe ${i + 1}/120:`, recipe.title);
  ...
  console.log('Migration complete. Generated 120 AI recipes from TheMealDB!');
}
