...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  // Only allow POST requests (cron jobs send POST)
  if (req.method !== 'POST') {
  ...
  try {
    ...
    if (countError) {
      throw new Error(`Failed to count existing AI recipes: ${countError.message}`);
    }
    ...
    const result = {
      success: true,
      message: 'Daily AI recipe generation complete!',
      data: {
        ...
        errors: errorCount,
        totalRecipes: currentCount + successCount
      }
    };
    ...
    console.error('Daily AI recipe generation failed:', error);
